...
public class TLJ2DStatusBubble {
	...
	public TLJ2DStatusBubble(double x, double y, double stemLength, double bubbleWidth, double bubbleHeight) {
		shape = createBubble(x, y, stemLength, bubbleWidth, bubbleHeight);
	}

	public TLJ2DStatusBubble(double x, double y, double stemLength, String text, double pad) {
		this(x, y, stemLength, text, pad, new Font(TLUtils.getDefaultFontFamily(), Font.PLAIN, 11));
	}

	public TLJ2DStatusBubble(double x, double y, double stemLength, String text, double pad, Font f) {
		textBounds = TLJ2DUtilities.getMultilineTextBounds(text, f, fontContext);
	...
	public void drawText(Graphics2D g) {
		if (text != null) {
			Font f = g.getFont();
			g.setFont(font);
			TLJ2DUtilities.drawMultilineString(g, text, textX, textY, TLJ2DUtilities.CENTER);
			g.setFont(f);
		}
	}
